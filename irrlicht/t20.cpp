...
class CMyLightManager: public scene::ILightManager, public IEventReceiver
{
    typedef enum
    {
        NO_MANAGEMENT, LIGHTS_NEAREST_NODE, LIGHTS_IN_ZONE
    } LightManagementMode;
...
public:
    CMyLightManager(scene::ISceneManager* sceneManager) :
            Mode(NO_MANAGEMENT), RequestedMode(NO_MANAGEMENT), SceneManager(sceneManager), SceneLightList(
                    0), CurrentRenderPass(scene::ESNRP_NONE), CurrentSceneNode(0)
    {
    }
    ...
    bool OnEvent(const SEvent & event)
    {
        ...
        if (event.EventType == irr::EET_KEY_INPUT_EVENT && event.KeyInput.PressedDown) {
            ...
            if (NO_MANAGEMENT == RequestedMode) SceneManager->setLightManager(0); // Show that it's safe to register the light manager
            else
    ...
    virtual void OnNodePreRender(scene::ISceneNode* node)
    {
        ...
        if (scene::ESNRP_SOLID != CurrentRenderPass) return;
        ...
        if (LIGHTS_NEAREST_NODE == Mode) {
            ...
            for (i = 0; i < SceneLightList->size(); ++i) {
                ...
                const f64 distance = lightNode->getAbsolutePosition().getDistanceFromSQ(
                        nodePosition);
            ...
            for (u32 i = 0; i < SceneLightList->size(); ++i) {
                if ((*SceneLightList)[i]->getType() != scene::ESNT_LIGHT) continue;
                scene::ILightSceneNode* lightNode =
                        static_cast<scene::ILightSceneNode*>((*SceneLightList)[i]);
                video::SLight & lightData = lightNode->getLightData();

                if (video::ELT_DIRECTIONAL != lightData.Type) lightNode->setVisible(false);
            }

            scene::ISceneNode * parentZone = findZone(node);
            if (parentZone) turnOnZoneLights(parentZone);
        }
    }
...
private:
    ...
    void turnOnZoneLights(scene::ISceneNode * node)
    {
        ...
        for (core::list<scene::ISceneNode*>::ConstIterator child = children.begin();
            ...
            if ((*child)->getType() == scene::ESNT_LIGHT) (*child)->setVisible(true);
            else
...
int t20()
{
    ...
    for (f32 zoneX = -100.f; zoneX <= 100.f; zoneX += 50.f)
        for (f32 zoneY = -60.f; zoneY <= 60.f; zoneY += 60.f) {
            ...
            smgr->addLightSceneNode(billboard, vector3df(0, 0, 0), video::SColorf(1, 0, 0),
                    lightRadius);
            ...
            billboard->setMaterialTexture(0, driver->getTexture("media/particle.bmp"));
            billboard->setMaterialFlag(video::EMF_LIGHTING, false);
            smgr->addLightSceneNode(billboard, vector3df(0, 0, 0), video::SColorf(0, 1, 0),
                    lightRadius);
            ...
            billboard->setMaterialTexture(0, driver->getTexture("media/particle.bmp"));
            billboard->setMaterialFlag(video::EMF_LIGHTING, false);
            smgr->addLightSceneNode(billboard, vector3df(0, 0, 0), video::SColorf(0, 0, 1),
                    lightRadius);

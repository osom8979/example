...
int t19()
{
    ...
    scene::ISceneNode * node = smgr->addMeshSceneNode(
            smgr->addArrowMesh("Arrow", video::SColor(255, 255, 0, 0),
                    video::SColor(255, 0, 255, 0), 16, 16, 2.f, 1.3f, 0.1f, 0.6f));
    ...
    while (device->run()) {
        ...
        if (joystickInfo.size() > 0) {
            ...
            if (fabs(moveHorizontal) < DEAD_ZONE) moveHorizontal = 0.f;

            moveVertical = (f32) joystickData.Axis[SEvent::SJoystickEvent::AXIS_Y] / -32767.f;
            if (fabs(moveVertical) < DEAD_ZONE) moveVertical = 0.f;
            ...
            if (povDegrees < 360) {
                ...
                else if (povDegrees > 270 || povDegrees < 90) moveVertical = +1.f;
            }
        ...
        if (!movedWithJoystick) {
            ...
            if (plane.getIntersectionWithLine(ray.start, ray.getVector(), mousePosition)) {
                ...
                if (toMousePosition.getLength() <= availableMovement) nodePosition = mousePosition; // Jump to the final position
                else

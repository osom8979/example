...
public:
    ...
    virtual bool OnEvent(const SEvent& event)
    {
        ...
        if (event.EventType == irr::EET_KEY_INPUT_EVENT) KeyIsDown[event.KeyInput.Key] =
...
int t04()
{
    ...
    if (n) {
        ...
        scene::ISceneNodeAnimator* anim = smgr->createFlyCircleAnimator(core::vector3df(0, 0, 30),
                20.0f);
        if (anim) {
            n->addAnimator(anim);
            anim->drop();
    ...
    gui::IGUIStaticText* diagnostics = device->getGUIEnvironment()->addStaticText(L"",
    ...
    while (device->run()) {
        ...
        if (receiver.IsKeyDown(irr::KEY_KEY_W)) nodePosition.Y += MOVEMENT_SPEED * frameDeltaTime;
        else if (receiver.IsKeyDown(irr::KEY_KEY_S)) nodePosition.Y -= MOVEMENT_SPEED
                * frameDeltaTime;

        if (receiver.IsKeyDown(irr::KEY_KEY_A)) nodePosition.X -= MOVEMENT_SPEED * frameDeltaTime;
        else if (receiver.IsKeyDown(irr::KEY_KEY_D)) nodePosition.X += MOVEMENT_SPEED
                * frameDeltaTime;

...
public:
    EventReceiver_basic(IrrlichtDevice *device) :
            Device(device)
    {
    }
...
IrrlichtDevice *startup()
{
    ...
    IGUIStaticText *text = guienv->addStaticText(L"FPS: 25", rect<s32>(140, 15, 200, 30), false,
            false, 0, 100);
...
int run(IrrlichtDevice *device)
{
    while (device->run())
        if (device->isWindowActive()) {
            ...
            IGUIElement *stat = device->getGUIEnvironment()->getRootGUIElement()->getElementFromId(
                    100);
            if (stat) {
                stringw str = L"FPS: ";
...
public:
    ...
    EventReceiver_terrain(IrrlichtDevice *device, scene::ISceneNode* terrain,
            scene::ISceneNode* skybox, scene::ISceneNode* skydome) :
...
int example_terrain()
{
    ...
    scene::ITerrainSceneNode* terrain = smgr->addTerrainSceneNode(
            "media/terrain-heightmap.bmp", 0,                  // parent node
            -1,                 // node id

...
private:
    ...
    core::array<f32> data;
public:
    HeightMap(u16 _w, u16 _h) :
            Width(_w), Height(_h), s(0.f), data(0)
    {
    ...
    void set(u16 x, u16 y, f32 z)
    {
        data[y * Width + x] = z;
    }
    void set(u32 i, f32 z)
    {
        data[i] = z;
    }
...
public:
    ...
    TMesh() :
            Mesh(0), Width(0), Height(0), Scale(1.f)
    {
        Mesh = new SMesh();
    }
    ...
    void init(const HeightMap &hm, f32 scale, colour_func cf, IVideoDriver *driver)
    {
        ...
        for (u32 y0 = 0; y0 < Height; y0 += sw) {
            u16 y1 = y0 + sw;
            if (y1 >= Height) y1 = Height - 1; // the last one might be narrower
...
int t23()
{
    ...
    IrrlichtDevice* device = createDevice(driverType, core::dimension2du(800, 600), 32, false,

...
const SItemElement * getItemElement(const stringc& key)
{
    ...
    while (item->key) {
        if (0 == strcmp(key.c_str(), item->key)) return item;
        item += 1;
    }
    return 0;
}
 ...
 */
void Q3ShaderFactory(Q3LevelLoadParameter &loadParam, IrrlichtDevice *device, IQ3LevelMesh* mesh,
        eQ3MeshIndex meshIndex, ISceneNode *parent, IMetaTriangleSelector *meta,
        bool showShaderName)
{
    if (0 == mesh || 0 == device) return;
    ...
    if (0 == additional_mesh || additional_mesh->getMeshBufferCount() == 0) return;
    ...
    IGUIFont *font = 0;
    if (showShaderName) font = device->getGUIEnvironment()->getFont("fontlucida.png");
    ...
    for (u32 i = 0; i != additional_mesh->getMeshBufferCount(); ++i) {
        ...
        if (0 == shader) {
            ...
            if (mat.getTexture(0) == 0) mat.setTexture(0, driver->getTexture("$blueimage"));
            if (mat.getTexture(1) == 0) mat.setTexture(1, driver->getTexture("$redimage"));
        ...
        if (showShaderName && node) {
            ...
            smgr->addBillboardTextSceneNode(font, (wchar_t*) buf, node,
                    dimension2d<f32>(80.0f, 8.0f), vector3df(0, 10, 0), sceneNodeID);
            sceneNodeID += 1;
        }
 ...
 */
void Q3ModelFactory(Q3LevelLoadParameter &loadParam, IrrlichtDevice *device,
        IQ3LevelMesh* masterMesh, ISceneNode *parent, bool showShaderName)
{
    if (0 == masterMesh) return;
    ...
    for (index = 0; (u32) index < entity.size(); ++index) {
        ...
        nodeCount += 1;
        for (u32 g = 0; g < 2; ++g) {
            if (0 == itemElement->model[g] || itemElement->model[g][0] == 0) continue;
            ...
            for (u32 j = 0; j != mesh->Buffer.size(); ++j) {
                ...
                if (0 == node) {
                    snprintf(buf, 128, "q3ModelFactory shader %s failed",
                            meshBuffer->Shader.c_str());
                ...
                if (itemElement->special & SPECIAL_SFX_ROTATE
                    ...
                    anim = smgr->createRotationAnimator(vector3df(0.f, 2.f, 0.f));
                ...
                if (itemElement->special & SPECIAL_SFX_BOUNCE) {
                    ...
                    anim = smgr->createFlyCircleAnimator(p + vector3df(0.f, 20.f, 0.f), 20.f,
                            0.005f, vector3df(1.f, 0.f, 0.f), core::fract(nodeCount * 0.05f), 1.f);
                    node->addAnimator(anim);
                    anim->drop();
                }
            }
        ...
        if (showShaderName) {
            ...
            smgr->addBillboardTextSceneNode(font, (wchar_t*) buf, parent,
                    dimension2d<f32>(80.0f, 8.0f), p + vector3df(0, 30, 0), 0);
        }
    }
 ...
 */
s32 Q3StartPosition(IQ3LevelMesh* mesh, ICameraSceneNode* camera, s32 startposIndex,

...
public:
    ...
    SettingManager(const stringw& settings_file) :
            SettingsFile(settings_file), NullDevice(0)
    {
    ...
    bool save()
    {
        ...
        for (; !i.atEnd(); i++) {
            ...
            xwriter->writeElement(L"setting", true, L"name", i->getKey().c_str(), L"value",
                    i->getValue().c_str());
...
{
    SAppContext() :
            Device(0), Gui(0), Driver(0), Settings(0), ShouldQuit(false), ButtonSave(0), ButtonExit(
                    0), ListboxDriver(0), ListboxResolution(0), CheckboxFullscreen(0)
    {
    }
        ...
        if (event.EventType == EET_GUI_EVENT) {
            ...
            case EGET_BUTTON_CLICKED: {
                ...
                if (event.GUIEvent.Caller == App.ButtonSave) {
                    ...
                    if (App.ListboxDriver->getSelected() != -1) App.Settings->setSetting(L"driver",
                            App.ListboxDriver->getListItem(App.ListboxDriver->getSelected()));

                    //if there is a selection write it
                    if (App.ListboxResolution->getSelected() != -1) App.Settings->setSetting(
                            L"resolution",
                            App.ListboxResolution->getListItem(
                                    App.ListboxResolution->getSelected()));
                    ...
                    if (App.Settings->save()) {
                        App.Gui->addMessageBox(L"settings save",
                                L"settings saved, please restart for settings to change effect", "",
                                true);
                    }
                }
...
void createSettingsDialog(SAppContext& app)
{
    ...
    gui::IGUIWindow* windowSettings = app.Gui->addWindow(rect<s32>(10, 10, 400, 400), true,
            L"Videosettings");
    app.Gui->addStaticText(L"Select your desired video settings", rect<s32>(10, 20, 200, 40), false,
            true, windowSettings);
    ...
    app.Gui->addStaticText(L"Resolution", rect<s32>(10, 130, 200, 140), false, true,
            windowSettings);
    app.ListboxResolution = app.Gui->addListBox(rect<s32>(10, 140, 220, 200), windowSettings, 1,
            true);
    ...
    app.CheckboxFullscreen = app.Gui->addCheckBox(app.Settings->getSettingAsBoolean("fullscreen"),
            rect<s32>(10, 220, 220, 240), windowSettings, -1, L"Fullscreen");

    //last but not least add save button
    app.ButtonSave = app.Gui->addButton(rect<s32>(80, 250, 150, 270), windowSettings, 2,
            L"Save video settings");

    //exit/cancel button
    app.ButtonExit = app.Gui->addButton(rect<s32>(160, 250, 240, 270), windowSettings, 2,
            L"Cancel and exit");
}
...
int t25()
{
    ...
    if (!app.Settings->load()) {
        ...
        map<stringw, s32>::Node* driver = app.Settings->DriverOptions.find(
                app.Settings->getSetting("driver"));

        if (driver) {
            if (irr::IrrlichtDevice::isDriverSupported(
        ...
        map<stringw, dimension2du>::Node* res = app.Settings->ResolutionOptions.find(
                app.Settings->getSetting("resolution"));
        if (res) {

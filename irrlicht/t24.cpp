...
enum ETimerAction
{
    ETA_MOUSE_VISIBLE, ETA_MOUSE_INVISIBLE,
};
...
{
    SAppContext() :
            Device(0), InfoStatic(0), EventBox(0), CursorBox(0), SpriteBox(0), ButtonSetVisible(0), ButtonSetInvisible(
                    0), ButtonSimulateBadFps(0), ButtonChangeIcon(0), SimulateBadFps(false)
    {
    }
...
void PrintMouseEventName(const SEvent& event, stringw &result)
{
    ...
    case EMIE_LMOUSE_PRESSED_DOWN:
        result += stringw(L"EMIE_LMOUSE_PRESSED_DOWN");
        break;
    case EMIE_RMOUSE_PRESSED_DOWN:
        result += stringw(L"EMIE_RMOUSE_PRESSED_DOWN");
        break;
    case EMIE_MMOUSE_PRESSED_DOWN:
        result += stringw(L"EMIE_MMOUSE_PRESSED_DOWN");
        break;
    case EMIE_LMOUSE_LEFT_UP:
        result += stringw(L"EMIE_LMOUSE_LEFT_UP");
        break;
    case EMIE_RMOUSE_LEFT_UP:
        result += stringw(L"EMIE_RMOUSE_LEFT_UP");
        break;
    case EMIE_MMOUSE_LEFT_UP:
        result += stringw(L"EMIE_MMOUSE_LEFT_UP");
        break;
    case EMIE_MOUSE_MOVED:
        result += stringw(L"EMIE_MOUSE_MOVED");
        break;
    case EMIE_MOUSE_WHEEL:
        result += stringw(L"EMIE_MOUSE_WHEEL");
        break;
    case EMIE_LMOUSE_DOUBLE_CLICK:
        result += stringw(L"EMIE_LMOUSE_DOUBLE_CLICK");
        break;
    case EMIE_RMOUSE_DOUBLE_CLICK:
        result += stringw(L"EMIE_RMOUSE_DOUBLE_CLICK");
        break;
    case EMIE_MMOUSE_DOUBLE_CLICK:
        result += stringw(L"EMIE_MMOUSE_DOUBLE_CLICK");
        break;
    case EMIE_LMOUSE_TRIPLE_CLICK:
        result += stringw(L"EMIE_LMOUSE_TRIPLE_CLICK");
        break;
    case EMIE_RMOUSE_TRIPLE_CLICK:
        result += stringw(L"EMIE_RMOUSE_TRIPLE_CLICK");
        break;
    case EMIE_MMOUSE_TRIPLE_CLICK:
        result += stringw(L"EMIE_MMOUSE_TRIPLE_CLICK");
        break;
    default:
        break;
    }
...
void PrintMouseState(const SEvent& event, stringw &result)
{
    ...
    if (event.MouseInput.Shift) result += stringw(L"true\n");
    else
        result += stringw(L"false\n");

    result += stringw(L"Control: ");
    if (event.MouseInput.Control) result += stringw(L"true\n");
    else
        result += stringw(L"false\n");
    ...
    if (event.MouseInput.isLeftPressed()) result += stringw(L"true\n");
    else
        result += stringw(L"false\n");

    result += stringw(L"isRightPressed: ");
    if (event.MouseInput.isRightPressed()) result += stringw(L"true\n");
    else
        result += stringw(L"false\n");

    result += stringw(L"isMiddlePressed: ");
    if (event.MouseInput.isMiddlePressed()) result += stringw(L"true\n");
    else
        result += stringw(L"false\n");
    ...
    virtual bool OnEvent(const SEvent& event)
    {
        ...
        if (event.EventType == EET_MOUSE_INPUT_EVENT) {
            ...
            if (event.MouseInput.Event != EMIE_MOUSE_MOVED
                    && event.MouseInput.Event != EMIE_MOUSE_WHEEL) // no spam
                            {
                infoText = L"";
...
s32 AddAnimatedIconToSpriteBank(gui::IGUISpriteBank * spriteBank, video::IVideoDriver* driver,
        const array<io::path>& files, u32 frameTime)
{
    if (!spriteBank || !driver || !files.size()) return -1;
 ...
 */
s32 AddAnimatedIconToSpriteBank(gui::IGUISpriteBank * spriteBank, video::IVideoDriver* driver,
        const io::path& file, const array<rect<s32> >& rects, u32 frameTime)
{
    if (!spriteBank || !driver || !rects.size()) return -1;
...
s32 AddIconToSpriteBank(gui::IGUISpriteBank * spriteBank, video::IVideoDriver* driver,
        const io::path& file, const core::rect<s32>& rect)
{
    if (!spriteBank || !driver) return -1;
...
int t24()
{
    ...
    context.ButtonSetVisible = env->addButton(rect<s32>(410, 20, 560, 40), 0, -1,
            L"set visible (delayed)");
    context.ButtonSetInvisible = env->addButton(rect<s32>(410, 50, 560, 70), 0, -1,
            L"set invisible (delayed)");
    context.ButtonSimulateBadFps = env->addButton(rect<s32>(410, 80, 560, 100), 0, -1,
            L"simulate bad FPS");
    context.ButtonSimulateBadFps->setIsPushButton(true);
    context.ButtonChangeIcon = env->addButton(rect<s32>(410, 140, 560, 160), 0, -1,
            L"replace cursor icon\n(cursor+sprite must be selected)");
    ...
    spriteCol.SpriteId = AddAnimatedIconToSpriteBank(SpriteBankIcons, driver,
            io::path("media/icon_crosshairs16x16col.png"), iconRects, 200);
    ...
    spriteNonAnimated.SpriteId = AddIconToSpriteBank(SpriteBankIcons, driver,
            io::path("media/icon_crosshairs16x16col.png"), rectIcon);
        ...
        // if (device->isWindowActive())
        {
            ...
            if (context.SpriteBox) {
                ...
                if (selectedSprite >= 0 && context.Sprites[selectedSprite].SpriteId >= 0) {
                    SpriteBankIcons->draw2DSprite(u32(context.Sprites[selectedSprite].SpriteId),
                            position2di(580, 140), 0, video::SColor(255, 255, 255, 255), 0,
                            realTimeNow);
                }
            }

 ...
 * @date   2018-01-15
 */
struct GpuIdWrapper : private Noncopyable
{
    GpuContext const & CONTEXT;
    GpuId id;

    GpuIdWrapper(GpuContext const & c, GpuId i = UNKNOWN_ID) : CONTEXT(c), id(i)
    { /* EMPTY. */ }
    ~GpuIdWrapper()
    { /* EMPTY. */ }

    inline bool isSameContext(GpuContext const & c) const TBAG_NOEXCEPT { return &CONTEXT == &c; }

    inline bool validate() const TBAG_NOEXCEPT { return id != UNKNOWN_ID; }
    inline bool validate(GpuContext const & c) const TBAG_NOEXCEPT { return isSameContext(c) && validate(); }

    inline void clearId() TBAG_NOEXCEPT { id = UNKNOWN_ID; }

    template <typename T> inline void setId(T i)    TBAG_NOEXCEPT { id = (GpuId)i; }

...
public:
    MyFrame(wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos =
...
public:
    ...
    wxSizeReportCtrl(wxWindow* parent, wxWindowID id = -1, const wxPoint& pos = wxDefaultPosition,
            const wxSize& size = wxDefaultSize, wxFrameManager* mgr = NULL) :
...
BEGIN_EVENT_TABLE(wxSizeReportCtrl, wxControl) EVT_PAINT(wxSizeReportCtrl::OnPaint)
...
private:
    ...
    wxBitmap CreateColorBitmap(const wxColour& c)
    {
        ...
        for (int x = 0; x < 25; ++x)
            for (int y = 0; y < 14; ++y) {
                wxColour pixcol = c;
                if (x == 0 || x == 24 || y == 0 || y == 13) pixcol = *wxBLACK;
    ...
    void UpdateColors()
    {
        ...
        wxColour cap = m_frame->GetDockArt()->GetColor(wxAUI_ART_CAPTION_COLOUR);
        m_caption_color->SetBitmapLabel(CreateColorBitmap(cap));

        wxColour captxt = m_frame->GetDockArt()->GetColor(wxAUI_ART_CAPTION_TEXT_COLOUR);
        m_caption_text_color->SetBitmapLabel(CreateColorBitmap(captxt));
    ...
    {
        m_frame->GetDockArt()->SetMetric(wxAUI_ART_PANE_BORDER_SIZE, event.GetPosition());
        m_frame->DoUpdate();
    }

    void OnSashSize(wxSpinEvent& event)
    {
        m_frame->GetDockArt()->SetMetric(wxAUI_ART_SASH_SIZE, event.GetPosition());
        m_frame->DoUpdate();
    }

    void OnCaptionSize(wxSpinEvent& event)
    {
        m_frame->GetDockArt()->SetMetric(wxAUI_ART_CAPTION_SIZE, event.GetPosition());
        m_frame->DoUpdate();
    }
    ...
    void OnSetColor(wxCommandEvent& event)
    {
        ...
        case ID_BackgroundColor:
            var = wxAUI_ART_BACKGROUND_COLOUR;
            break;
        case ID_SashColor:
            var = wxAUI_ART_SASH_COLOUR;
            break;
        case ID_CaptionColor:
            var = wxAUI_ART_CAPTION_COLOUR;
            break;
        case ID_CaptionTextColor:
            var = wxAUI_ART_CAPTION_TEXT_COLOUR;
            break;
        case ID_BorderColor:
            var = wxAUI_ART_BORDER_COLOUR;
            break;
        case ID_GripperColor:
            var = wxAUI_ART_GRIPPER_COLOUR;
            break;
        default:
            return;
        }
...
private:
    ...
    wxBitmapButton* m_caption_text_color;
    wxBitmapButton* m_caption_color;
...
BEGIN_EVENT_TABLE(SettingsPanel, wxPanel) EVT_SPINCTRL(ID_PaneBorderSize, SettingsPanel::OnPaneBorderSize)
...
EVT_BUTTON(ID_CaptionColor, SettingsPanel::OnSetColor)
EVT_BUTTON(ID_CaptionTextColor, SettingsPanel::OnSetColor)
EVT_BUTTON(ID_BorderColor, SettingsPanel::OnSetColor)
EVT_BUTTON(ID_GripperColor, SettingsPanel::OnSetColor)
END_EVENT_TABLE()

BEGIN_EVENT_TABLE(MyFrame, wxFrame) EVT_ERASE_BACKGROUND(MyFrame::OnEraseBackground)
        ...
        wxFrame(parent, id, title, pos, size, style)
{
    ...
    m_mgr.AddPane(CreateSizeReportCtrl(),
            wxPaneInfo().Name(wxT("test1")).Caption(wxT("Pane Caption")).Top());

    m_mgr.AddPane(CreateSizeReportCtrl(),
            wxPaneInfo().Name(wxT("test2")).Caption(wxT("Client Size Reporter")).Bottom().Position(
                    1));

    m_mgr.AddPane(CreateSizeReportCtrl(),
            wxPaneInfo().Name(wxT("test3")).Caption(wxT("Client Size Reporter")).Bottom());

    m_mgr.AddPane(CreateSizeReportCtrl(),
            wxPaneInfo().Name(wxT("test4")).Caption(wxT("Pane Caption")).Left());

    m_mgr.AddPane(CreateSizeReportCtrl(),
            wxPaneInfo().Name(wxT("test5")).Caption(wxT("Pane Caption")).Right());

    m_mgr.AddPane(CreateSizeReportCtrl(),
            wxPaneInfo().Name(wxT("test6")).Caption(wxT("Client Size Reporter")).Right().Row(1));

    m_mgr.AddPane(CreateSizeReportCtrl(),
            wxPaneInfo().Name(wxT("test7")).Caption(wxT("Client Size Reporter")).Left().Layer(1));

    m_mgr.AddPane(CreateTreeCtrl(),
            wxPaneInfo().Name(wxT("test8")).Caption(wxT("Tree Pane")).Left().Layer(1).Position(1));

    m_mgr.AddPane(CreateSizeReportCtrl(),
            wxPaneInfo().Name(wxT("test9")).Caption(wxT("Min Size 200x100")).BestSize(
                    wxSize(200, 100)).MinSize(wxSize(200, 100)).Bottom().Layer(1));

    m_mgr.AddPane(CreateTextCtrl(),
            wxPaneInfo().Name(wxT("test10")).Caption(wxT("Text Pane")).Bottom().Layer(1).Position(
                    1));

    m_mgr.AddPane(CreateSizeReportCtrl(),
            wxPaneInfo().Name(wxT("test11")).Caption(wxT("Fixed Pane")).Bottom().Layer(1).Position(
                    2).Fixed());

    m_mgr.AddPane(new SettingsPanel(this, this),
            wxPaneInfo().Name(wxT("settings")).Caption(wxT("Dock Manager Settings")).Dockable(
                    false));

    // create some center panes

    m_mgr.AddPane(CreateGrid(), wxPaneInfo().Name(wxT("grid_content")).CenterPane().Hide());

    m_mgr.AddPane(CreateTreeCtrl(), wxPaneInfo().Name(wxT("tree_content")).CenterPane().Hide());

    m_mgr.AddPane(CreateSizeReportCtrl(),
            wxPaneInfo().Name(wxT("sizereport_content")).CenterPane().Hide());

    m_mgr.AddPane(CreateTextCtrl(), wxPaneInfo().Name(wxT("text_content")).CenterPane().Hide());

    m_mgr.AddPane(CreateHTMLCtrl(), wxPaneInfo().Name(wxT("html_content")).CenterPane());

    // add the toolbars to the manager

    m_mgr.AddPane(tb1,
            wxPaneInfo().Name(wxT("tb1")).Caption(wxT("Big Toolbar")).ToolbarPane().Top().LeftDockable(
                    false).RightDockable(false));

    m_mgr.AddPane(tb2,
            wxPaneInfo().Name(wxT("tb2")).Caption(wxT("Toolbar 2")).ToolbarPane().Top().Row(1).LeftDockable(
                    false).RightDockable(false));

    m_mgr.AddPane(tb3,
            wxPaneInfo().Name(wxT("tb3")).Caption(wxT("Toolbar 3")).ToolbarPane().Top().Row(1).Position(
                    1).LeftDockable(false).RightDockable(false));

    m_mgr.AddPane(tb4,
            wxPaneInfo().Name(wxT("tb4")).Caption(wxT("Sample Bookmark Toolbar")).ToolbarPane().Top().Row(
                    2).LeftDockable(false).RightDockable(false));

    m_mgr.AddPane(new wxButton(this, -1, _("Test Button")),
            wxPaneInfo().Name(wxT("tb5")).ToolbarPane().Top().Row(2).Position(1).LeftDockable(false).RightDockable(
                    false));
...
void MyFrame::OnSettings(wxCommandEvent& event)
{
    ...
    if (floating_pane.floating_pos == wxDefaultPosition) floating_pane.FloatingPosition(
            GetStartPosition());
...
void MyFrame::OnManagerFlag(wxCommandEvent& event)
{
    ...
    case ID_TransparentHintFade:
        flag = wxAUI_MGR_TRANSPARENT_HINT_FADE;
        break;
    }
...
{
    m_mgr.AddPane(CreateTreeCtrl(),
            wxPaneInfo().Name(wxT("Test")).Caption(wxT("Tree Control")).Float().FloatingPosition(
                    GetStartPosition()).FloatingSize(wxSize(150, 300)));
    m_mgr.Update();
}
...
void MyFrame::OnCreateGrid(wxCommandEvent& event)
{
    m_mgr.AddPane(CreateGrid(),
            wxPaneInfo().Name(wxT("Test")).Caption(wxT("Grid")).Float().FloatingPosition(
                    GetStartPosition()).FloatingSize(wxSize(300, 200)));
    m_mgr.Update();
}

void MyFrame::OnCreateHTML(wxCommandEvent& event)
{
    m_mgr.AddPane(CreateHTMLCtrl(),
            wxPaneInfo().Name(wxT("Test")).Caption(wxT("Grid")).Float().FloatingPosition(
                    GetStartPosition()).FloatingSize(wxSize(300, 200)));
    m_mgr.Update();
}
...
{
    m_mgr.AddPane(CreateTextCtrl(),
            wxPaneInfo().Name(wxT("Test")).Caption(wxT("Text Control")).Float().FloatingPosition(
                    GetStartPosition()));
    m_mgr.Update();
}
...
{
    m_mgr.AddPane(CreateSizeReportCtrl(),
            wxPaneInfo().Name(wxT("Test")).Caption(wxT("Client Size Reporter")).Float().FloatingPosition(
                    GetStartPosition()));
    m_mgr.Update();
}
...
void MyFrame::OnChangeContentPane(wxCommandEvent& event)
{
    ...
    m_mgr.GetPane(wxT("sizereport_content")).Show(
            event.GetId() == ID_SizeReportContent ? true : false);
...
wxTextCtrl* MyFrame::CreateTextCtrl()
{
    ...
    return new wxTextCtrl(this, -1, text, wxPoint(0, 0), wxSize(150, 90),
    wxNO_BORDER | wxTE_MULTILINE);
}

wxGrid* MyFrame::CreateGrid()
{
    wxGrid* grid = new wxGrid(this, -1, wxPoint(0, 0), wxSize(150, 250),
...
{
    wxTreeCtrl* tree = new wxTreeCtrl(this, -1, wxPoint(0, 0), wxSize(160, 250),
...
{
    wxSizeReportCtrl* ctrl = new wxSizeReportCtrl(this, -1, wxDefaultPosition,
...
{
    wxHtmlWindow* ctrl = new wxHtmlWindow(this, -1, wxDefaultPosition, wxSize(400, 300));
...
{
    const char* text = "<html><body><h3>Welcome to wxAUI 0.9</h3><br/>"
            "<b>Overview</b><br/>"
            "<p>wxAUI is an Advanced User Interface library for the wxWidgets toolkit "
            "that allows developers to create high-quality, cross-platform user "
            "interfaces quickly and easily.</p><br/>"
            "<p>With wxAUI version 0.9, developers can create application frameworks with:</p><ul>"
            "<li>Native, dockable floating frames</li>"
            "<li>Native toolbars incorporating real-time, &quot;spring-loaded&quot; dragging</li>"
            "<li>Customizable floating/docking behavior</li>"
            "<li>Completely customizable look-and-feel</li>"
            "<li>Perspective saving and loading</li></ul></body></html>";
...
int t51()
{
    wxFrame* frame = new MyFrame(NULL, wxID_ANY, wxT("Frame Manager Test"), wxDefaultPosition,
            wxSize(800, 600));
    // SetTopWindow(frame);
    frame->Show();
    return EXIT_SUCCESS;

...
FILE_NAME=pthreads-w32-2-9-1-release
FILE_EXT=.tar.gz
...
checkExitError
echo "- build."
$MAKE clean GC GCE GC-inlined GCE-inlined GC-static -f GNUmakefile >> $LOG_PATH

checkExitError
echo "- install."
mkdir -p $PREFIX/doc/$FILE_NAME
cp pthread*.dll $LBIN
cp pthread.h sched.h semaphore.h $LINC
cp *pthread*.a *pthread*.def $LLIB
cp -r manual tests $PREFIX/doc/$FILE_NAME

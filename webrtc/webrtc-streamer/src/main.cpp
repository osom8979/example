...
int main(int argc, char* argv[])
{
				...
				if (root.isMember("urls")) {
					...
					for( auto it = urls.begin() ; it != urls.end() ; it++ ) {
						...
						if (value.isMember("video")) {
							urlVideoList.insert(std::make_pair(name, std::string(value["video"].asString())));
						} 
						if (value.isMember("audio")) {
							urlAudioList.insert(std::make_pair(name, std::string(value["audio"].asString())));
						}
					}
				}
				break;
	...
	for (auto & video_item : urlVideoList) {
		std::cout << "Video: " << video_item.first << ", " << video_item.second << std::endl;
	}
